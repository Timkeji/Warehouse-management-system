...
function hwc_outmoreinfo(self){
    var shipment_id=$(self).parent().prev().text()
    console.log(shipment_id)
    $.get(SERVER_PATH+'/api/superadmin/outboundinformationmax?shipment_id='+shipment_id, function (data) {
        let Str = '';
...
$.get(SERVER_PATH+'/api/superadmin/outboundinformation', function (data) {
    console.log(data)
    let Str = '';
    for (var i = 0; i < data.data.length; i++) {
        $(data.data[i]).each(function () {
           ...
           <tr>
             ...
             <td><span>出库</span></td>
             <td style="display: none">${data.data[i].shipment_id}</td>
             <td class="info"><span class="pass" onclick="hwc_outmoreinfo(this)">详情</span></td>
           </tr>`;
        });
    }
    $('#form').empty();
...
function hwc_outboundSeI(){
    var impor = document.getElementById("import").value;
    $.get(SERVER_PATH+'/api/superadmin/outboundseinformation?product_id='+impor, function (data) {
        console.log(data)
        let Str = '';
        for (var i = 0; i < data.data.length; i++) {
            $(data.data[i]).each(function () {
